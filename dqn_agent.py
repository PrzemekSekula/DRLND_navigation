...
class Agent():
    ...
    def act(self, state, eps=0., train_mode = True):
        ...
        ======
            ...
            train_mode: - if False, agent is not learning
        """
    ...
    def learn(self, experiences, gamma):
        ...
        td_target = rewards + ( (1 - dones) * gamma * qmax_next )
        
        q_local = self.qnetwork_local(states) # all possible actions
        q_local = q_local.gather(1, actions) # only selected actions
        
        loss = F.mse_loss(td_target, q_local)
        loss.backward()
        self.optimizer.step()
